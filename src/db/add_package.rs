...
pub(crate) fn add_release_into_database(
    ...
    let result = conn.query_one(
        "INSERT INTO releases (
            ...
            is_library,
            documentation_url, features,
            repository_id, archive_storage
         )
         VALUES (
            $1,  $2,  $3,  $4,  $5,  $6,  'in_progress', $7,  $8,  $9,
    ...
    let release_id: i32 = result.get("id");
...
/// Updates a release in the database after a build completes.
pub(crate) fn update_release_in_database(
    conn: &mut Client,
    release_id: i32,
    ...
    debug!("Updating release in database");
    ...
    if result == 0 {
        return Err(anyhow!("Updating unknown release {}", release_id));
    }
...
pub(crate) fn update_build_in_database(
    conn: &mut Client,
    id: i32,
    status: BuildStatus,
) -> Result<()> {
    let count = conn.execute(
        "UPDATE builds SET build_status = $2 WHERE id = $1",
        &[&id, &status])?;
    if count == 0 {
        return Err(anyhow!("Unknown build {}", id));
    }
    Ok(())
}
...
pub(crate) fn add_compression_into_database(conn: &mut Client, release_id: i32, algorithm: CompressionAlgorithm) -> Result<()> {
    conn.execute(
        "INSERT INTO compression_rels (release, algorithm)
         VALUES ($1, $2)
         ON CONFLICT DO NOTHING;",
         &[&release_id, &(algorithm as i32)],
    )?;
    Ok(())
}
