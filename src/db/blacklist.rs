...
pub fn is_blacklisted(conn: &Connection, name: &str) -> anyhow::Result<bool> {
    let rows = conn.query(
...
pub fn list_crates(conn: &Connection) -> anyhow::Result<Vec<String>> {
    let rows = conn.query(
...
pub fn add_crate(conn: &Connection, name: &str) -> anyhow::Result<()> {
...
pub fn remove_crate(conn: &Connection, name: &str) -> anyhow::Result<()> {
    if !is_blacklisted(conn, name)? {
